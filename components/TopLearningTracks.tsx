...
const TopCourseCard = ({ course, index }: { course: Course; index: number }) => {
  ...
  useEffect(() => {
    const fetchTopCourses = async () => {
      try {
        ...
        const response = await fetch(`https://${projectId}.supabase.co/functions/v1/make-server-9180a2e7/courses/selection/top`, {
          headers: {
    ...
    return (
      ...
      >
        ...
        <div className="px-4 md:px-8 lg:px-12 xl:px-16">
          <p className="text-white/60">Unable to load top courses. Please try again later.</p>
        </div>
      </motion.div>
    );
  }
      ...
      <div className="relative">
        ...
        >
          {topCourses.map((course, index) => (
            <TopCourseCard key={course.id} course={course} index={index} />
          ))}
        </motion.div>
      </div>
    </motion.div>
