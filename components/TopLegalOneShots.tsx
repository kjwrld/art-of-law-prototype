...
const TopLegalOneShotCard = ({ course, index }: { course: Course; index: number }) => {
...
export const TopLegalOneShots = () => {
  const [topLegalOneShots, setTopLegalOneShots] = useState<Course[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    const fetchTopLegalOneShots = async () => {
      try {
        setLoading(true);
        setError(null);
        
        console.log('🎬 Fetching Top 10 Legal One-Shots from database...');
        const response = await fetch(`https://${projectId}.supabase.co/functions/v1/make-server-9180a2e7/courses/selection/legal_one_shots`, {
          headers: {
        ...
        if (data.success && data.courses) {
          ...
          console.error('Failed to fetch top legal one-shots:', data.error);
    ...
    return (
      ...
      >
        ...
        <div className="px-4 md:px-8 lg:px-12 xl:px-16">
          <p className="text-white/60">Unable to load top legal one-shots. Please try again later.</p>
        </div>
      </motion.div>
    );
  }
  ...
  return (
    ...
    >
      ...
      {/* Scrollable Legal One-Shot Cards */}
      <div className="relative">
        ...
        >
          {topLegalOneShots.map((course, index) => (
            <TopLegalOneShotCard key={course.id} course={course} index={index} />
          ))}
        </motion.div>
      </div>
    </motion.div>
