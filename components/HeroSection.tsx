...
import heroImage from "../src/assets/placeholder.svg";
...
export function HeroSection({ onCourseClick }: HeroSectionProps) {
  ...
  useEffect(() => {
    const fetchCelebrityCourses = async () => {
      try {
        console.log('🌟 Fetching Celebrity courses for hero slideshow...');
        const response = await fetch(`https://${projectId}.supabase.co/functions/v1/make-server-9180a2e7/courses/selection/Celebrity`, {
          headers: {
        ...
        if (data.success && data.courses) {
          ...
          console.log(`✅ Loaded ${data.courses.length} celebrity courses for hero slideshow`);
          ...
          const celebritySlides: HeroSlide[] = data.courses.map((course: Course) => ({
            id: course.id,
            title: course.title,
          ...
          console.error('Failed to fetch celebrity courses:', data.error);
  ...
  return (
    <section ref={heroRef} className="relative h-[67vh] md:h-[67vh] w-full overflow-hidden">
      ...
      {slides.length > 1 && (
        <div className="absolute bottom-4 left-1/2 -translate-x-1/2 z-15">
          ...
          <div className="w-20 h-1 bg-white/20 rounded-full overflow-hidden">
            <motion.div
              className="h-full bg-[var(--aow-gold)] rounded-full"
        ...
        <div className="flex-1 flex items-end pb-20 md:pb-16">
          <div className="w-full">
            <div className="max-w-5xl mx-auto md:mx-0 md:max-w-4xl text-center md:text-left md:pl-8 lg:pl-12 xl:pl-16">
              ...
              {currentSlideData.showBadge && currentSlide !== 0 && (
                <AnimatePresence mode="wait">
                  <motion.div
              ...
              {currentSlide === 0 ? (
                <div className="mb-4 md:mb-6">
                  <h1 className="text-white mb-4 font-['Alacrity_Sans',_sans-serif] leading-[122.8%]" style={{ textShadow: '0 1px 3px rgba(0, 0, 0, 0.6)' }}>
                    {currentSlideData.title.split('\n').map((line, index) => (
                      <span key={index} className="block text-4xl sm:text-5xl md:text-6xl lg:text-7xl xl:text-8xl font-normal tracking-tight lg:whitespace-nowrap">
                        {line}
                      </span>
                    ))}
                  </h1>
                ...
                <AnimatePresence mode="wait">
                  ...
                  >
                    <h1 className="text-white mb-4 font-['Alacrity_Sans',_sans-serif] leading-[122.8%]" style={{ textShadow: '0 1px 3px rgba(0, 0, 0, 0.6)' }}>
                      {currentSlideData.title.split('\n').map((line, index) => (
                        <span key={index} className="block text-2xl sm:text-3xl md:text-4xl lg:text-5xl xl:text-6xl font-normal tracking-tight">
                          {line}
                        </span>
                      ))}
                    </h1>
        ...
        <div className="pb-12 md:pb-0">
          ...
          <div className="md:hidden flex justify-center">
            ...
            >
              ...
              <div className="font-['Manrope',_sans-serif] font-bold leading-[0] relative shrink-0 text-[var(--aow-black)] text-[13px] text-left text-nowrap">
          ...
          <div className="hidden md:block absolute bottom-8 right-8 lg:bottom-12 lg:right-12">
            ...
            >
              ...
              <div className="font-['Manrope',_sans-serif] font-bold leading-[0] relative shrink-0 text-[var(--aow-black)] text-[14px] text-left text-nowrap">
                <p className="block leading-none whitespace-pre">Play Now</p>
              </div>
            </motion.div>
          </div>
