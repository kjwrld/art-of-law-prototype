...
export function MyCourses({ onNavigate }: MyCoursesProps) {
  ...
  const [continueWatchingCourses, setContinueWatchingCourses] = useState<Course[]>([]);
  const [loading, setLoading] = useState(true);

  // Fetch bookmarked and continue watching courses
  useEffect(() => {
    const fetchMyCourses = async () => {
      try {
        // For now, we'll use placeholder data
        // TODO: Implement actual bookmarking system with Supabase

        // Fetch "Continue" courses for Continue Watching section
        const continueResponse = await fetch(`https://${projectId}.supabase.co/functions/v1/make-server-9180a2e7/courses/selection/Continue`, {
          headers: {
        ...
        if (continueData.success && continueData.courses) {
          setContinueWatchingCourses(continueData.courses);
        }

        // TODO: Fetch actual bookmarked courses
        // For now, show empty state for bookmarks
          ...
          {bookmarkedCourses.length > 0 ? (
            ...
            <div className="bg-[#1a1a1a] rounded-2xl border border-aow-gold/10 p-8 text-center">
              ...
              <p className="text-white/60 font-['Luxora_Grotesk',_sans-serif] max-w-md mx-auto">
                Start building your personal library by bookmarking courses you want to watch later. Look for the bookmark icon on any course.
              </p>
            </div>
          )}
        </section>
        ...
        <section>
          ...
          {continueWatchingCourses.length > 0 ? (
            ...
            <div className="bg-[#1a1a1a] rounded-2xl border border-aow-gold/10 p-8 text-center">
              ...
              <p className="text-white/60 font-['Luxora_Grotesk',_sans-serif] max-w-md mx-auto">
                Start watching a course to see your progress here. Your learning journey awaits!
              </p>
            </div>
          )}
        </section>
        ...
        <section>
          <div className="bg-gradient-to-br from-[#1a1a1a] to-[#0d0d0d] rounded-2xl border border-aow-gold/10 p-8 text-center">
            ...
            <p className="text-white/60 font-['Luxora_Grotesk',_sans-serif] mb-4 max-w-md mx-auto">
              Follow structured pathways designed to elevate your legal expertise systematically.
            </p>
            <button 
