...
export function Navigation({ currentPage, onPageChange, onMenuToggle }: NavigationProps) {
  ...
  return (
    <motion.nav 
      ...
      className="fixed top-0 left-0 right-0 z-50 bg-[var(--aow-black)]/95 backdrop-blur-xl border-b border-[var(--aow-gold)]/10"
    >
      {/* Centered container with constrained width */}
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-22">
          
          {/* LEFT SIDE - All the way to the left */}
          <div className="flex items-center space-x-4">
            ...
            <button
              ...
              className="relative flex items-center space-x-3 cursor-pointer group min-w-fit pb-2"
            >
          ...
          <div className="flex items-center space-x-8 ml-12">
              ...
              {navigationItems.map((item) => (
                ...
                >
                  <item.icon className={`h-4 w-4 transition-colors duration-300 ${
                    currentPage === item.page ? 'text-[var(--aow-gold)]' : ''
                  }`} />
                  <span>{item.name}</span>
                  {/* Gold Underline for Active State */}
                  <motion.div
                    className="absolute bottom-0 left-1/2 h-0.5 bg-[var(--aow-gold)] rounded-full"
                    initial={{ width: 0, x: '-50%' }}
                    animate={{ 
                      width: currentPage === item.page ? '80%' : 0,
                      x: '-50%'
                    }}
                    transition={{ duration: 0.3, ease: "easeOut" }}
                  />
            ...
            <div className="hidden md:flex lg:hidden items-center space-x-6">
              {navigationItems.map((item) => (
                ...
                >
                  ...
                  {/* Gold indicator dot */}
                  <motion.div
                    className="absolute bottom-0 w-1 h-1 bg-[var(--aow-gold)] rounded-full"
                    initial={{ opacity: 0 }}
                    animate={{ 
            ...
            <DropdownMenu>
                ...
                >
                  ...
                  >
                    <div className="relative h-7 w-7 rounded-full bg-white/10 flex items-center justify-center border border-white/20">
                      ...
                      <div className="absolute -bottom-0.5 -right-0.5 h-2.5 w-2.5 bg-green-400 border border-[var(--aow-black)] rounded-full"></div>
                    </div>
                    <div className="hidden xl:flex flex-col items-start">
                      <span className="text-white text-sm font-medium">Eric Jarrett</span>
                      <span className="text-white/60 text-xs">Free Trial</span>
                    </div>
                    <ChevronDown className="h-4 w-4 text-white/60 group-hover:text-white transition-colors duration-200 hidden lg:block" />
                    {/* Notification Indicator */}
                    <Badge 
                      variant="destructive" 
                      className="absolute -top-1 -right-1 h-4 w-4 p-0 text-xs bg-[var(--aow-gold)] text-[var(--aow-black)] border-0 flex items-center justify-center"
                    >
                      3
                    </Badge>
                  </Button>
              ...
              <DropdownMenuContent 
                className="w-64 bg-[var(--aow-black)]/95 backdrop-blur-xl border border-[var(--aow-gold)]/20 shadow-2xl" 
                align="end"
                sideOffset={8}
              >
                <div className="px-3 py-3 border-b border-[var(--aow-gold)]/20">
                  <div className="flex items-center space-x-3">
                    <div className="h-10 w-10 rounded-full bg-white/10 flex items-center justify-center border border-white/20">
                      <User className="h-5 w-5 text-white" />
                    </div>
                    <div>
                      <div className="text-white font-medium">Eric Jarrett</div>
                      <div className="text-white/60 text-sm">eric@jarrettlaw.com</div>
                    </div>
                  </div>
                </div>
                
                {/* Mobile Navigation Menu Items */}
                <div className="md:hidden py-2 border-b border-[var(--aow-gold)]/20">
                  {navigationItems.map((item) => (
                    <DropdownMenuItem 
                      key={item.name}
                      onClick={() => onPageChange(item.page)}
                      className={`text-white hover:bg-white/10 hover:text-white transition-colors duration-200 ${
                        currentPage === item.page ? 'bg-white/15 text-white' : ''
                      }`}
                    >
                ...
                <div className="py-2 border-b border-[var(--aow-gold)]/20">
                  <DropdownMenuItem className="text-white hover:bg-white/10 hover:text-white transition-colors duration-200">
                    <div className="flex items-center justify-between w-full">
                      ...
                      <Badge 
                        variant="destructive" 
                        className="h-5 w-5 p-0 text-xs bg-[var(--aow-gold)] text-[var(--aow-black)] border-0 flex items-center justify-center"
                      >
                        3
                      </Badge>
                    </div>
                ...
                <DropdownMenuSeparator className="bg-[var(--aow-gold)]/20" />
