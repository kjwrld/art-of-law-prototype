...
export function LearningTracks() {
  ...
  const [learningTracksCourses, setLearningTracksCourses] = useState<Course[]>([]);
  ...
  useEffect(() => {
    const fetchLearningTracks = async () => {
      try {
        setLoading(true);
        const response = await fetch(`https://${projectId}.supabase.co/functions/v1/make-server-9180a2e7/courses/selection/learning_tracks`, {
          headers: {
    ...
    <div className="min-h-screen bg-[var(--aow-black)] text-white pt-20">
      ...
      <div className="px-6 md:px-8 lg:px-12 xl:px-16 pt-8 pb-6">
        ...
        <p className="text-white/70 text-lg font-['Luxora_Grotesk',_sans-serif]">
          Structured pathways to legal mastery, curated by industry leaders
        </p>
      </div>
            ...
            <div className="relative">
              ...
              >
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="text-center">
                        <h3 className="text-white font-semibold text-sm mb-1 leading-tight font-['Luxora_Grotesk:Medium',_sans-serif] group-hover/track-card:text-[var(--aow-gold-light)] transition-colors duration-300 max-w-[140px]">
                          {course.title}
                        </h3>
                ...
                <motion.div
                  initial={{ opacity: 0, x: 50 }}
                  animate={{ opacity: 1, x: 0 }}
                  transition={{ duration: 0.6, delay: 0.1 * learningTracksCourses.length, ease: "easeOut" }}
                  className="flex-shrink-0 flex flex-col items-center group/placeholder cursor-default"
                >
                  <div className="relative mb-4">
                    <div className="relative">
                      <div className="w-32 h-44 rounded-lg border-2 border-dashed border-white/30 flex items-center justify-center group-hover/placeholder:border-[var(--aow-gold)]/50 transition-colors duration-300">
                        <div className="text-center">
                          <div className="w-16 h-16 bg-white/5 rounded-full flex items-center justify-center border border-white/20 mb-3 group-hover/placeholder:border-[var(--aow-gold)]/40 transition-colors duration-300">
                            <Sprout className="w-8 h-8 text-white/40 group-hover/placeholder:text-[var(--aow-gold)]/60 transition-colors duration-300" />
                          </div>
                  ...
                  {/* Placeholder Title */}
                  <div className="text-center">
                    <h3 className="text-white/60 font-semibold text-sm mb-1 leading-tight font-['Luxora_Grotesk:Medium',_sans-serif] group-hover/placeholder:text-[var(--aow-gold-light)] transition-colors duration-300 max-w-[140px]">
        ...
        <section className="px-6 md:px-8 lg:px-12 xl:px-16">
          ...
          <div className="relative h-10 mb-0 lg:h-10">
            <div className="absolute right-8 bottom-0 z-20 flex px-6 lg:px-0">
              ...
              <div 
                className={`folder-tab ${activeTab === 'master' ? 'active' : 'inactive'}`}
                onClick={() => setActiveTab('master')}
              >
                <span className="folder-tab-text whitespace-nowrap">
                  {/* Mobile: Show icon only, Desktop: Show full text */}
                  <span className="hidden lg:inline">Master the Courtroom</span>
              ...
              {/* Make the Leap - Tab */}
              <div 
                className={`folder-tab ${activeTab === 'leap' ? 'active' : 'inactive'}`}
                onClick={() => setActiveTab('leap')}
              >
                <span className="folder-tab-text whitespace-nowrap">
                  {/* Mobile: Show icon only, Desktop: Show full text */}
                  <span className="hidden lg:inline">Make the Leap: Leaving the Firm to Start Your Own</span>
                  <span className="lg:hidden flex items-center justify-center">
              ...
              <div 
                className={`folder-tab ${activeTab === 'leader' ? 'active' : 'inactive'}`}
                onClick={() => setActiveTab('leader')}
              >
                <span className="folder-tab-text whitespace-nowrap">
                  {/* Mobile: Show icon only, Desktop: Show full text */}
                  <span className="hidden lg:inline">Become a Thought Leader</span>
                  <span className="lg:hidden flex items-center justify-center">
          ...
          <div className="bg-gradient-to-br from-[#1a1a1a] to-[#0d0d0d] rounded-2xl border border-[var(--aow-gold)]/10 p-6 md:p-8">
            
            {/* Recommended Track Header */}
            <div className="flex items-center gap-3 mb-6">
              <div className="w-10 h-10 bg-[var(--aow-gold)] rounded-lg flex items-center justify-center">
                <currentTrack.icon className="w-6 h-6 text-[var(--aow-black)]" />
              </div>
            ...
            <div className="space-y-6">
              <div className="flex items-center justify-between text-sm">
                ...
                <span className="text-[var(--aow-gold)] font-['Luxora_Grotesk',_sans-serif] font-medium">
                  {currentTrack.progress.completed} of {currentTrack.progress.total} completed
                </span>
              </div>
              ...
              <div className="relative">
                ...
                {activeTab === 'master' && (
                  <div 
                    className="absolute left-8 w-0.5 bg-[var(--aow-gold)] z-0 transition-all duration-500"
                    style={{ 
                ...
                <div className="space-y-8 pb-4">
                  {currentTrack.courses.map((course, index) => (
                    <CourseProgressItem 
                      ...
                      course={course} 
                      index={index}
                      onCourseClick={handleTrackCourseClick}
                    />
                  ))}
                </div>
              </div>
        ...
        <section className="mt-12 px-6 md:px-8 lg:px-12 xl:px-16">
          <div className="text-center">
            <div className="w-16 h-16 bg-white/10 rounded-full flex items-center justify-center mx-auto mb-4">
              <Sprout className="w-8 h-8 text-[var(--aow-gold)]" />
            </div>
            ...
            <p className="text-white/60 font-['Luxora_Grotesk',_sans-serif] max-w-md mx-auto">
              We're crafting additional learning pathways across specialized practice areas. Each track is designed to elevate your expertise systematically.
            </p>
          </div>
        </section>
