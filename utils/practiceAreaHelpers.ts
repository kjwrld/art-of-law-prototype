...
import { FOCUS_MAP } from "../constants/practiceArea";
    ...
    for (const [category, focusValue] of Object.entries(FOCUS_MAP)) {
        try {
            ...
            const response = await fetch(
                `https://${projectId}.supabase.co/functions/v1/make-server-9180a2e7/courses?focus=${focusValue}`,
                {
                    headers: {
            ...
            if (data.success && data.courses) {
                ...
                console.warn(
                    `⚠️ No courses found for ${category} (focus: ${focusValue})`
                );
            }
        } catch (error) {
...
export const filterCoursesByFocus = (courses: any[], activeTabs: string[]) => {
    const selectedFocusAreas = activeTabs
        .map((tab) => FOCUS_MAP[tab as keyof typeof FOCUS_MAP])
        .filter(Boolean);
