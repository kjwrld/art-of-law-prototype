...
app.get("/make-server-9180a2e7/health", (c) => {
    return c.json({ status: "ok", message: "Art of Law Legal API is running" });
});
    ...
    try {
        ...
        for (const selection of selections) {
            const selectionCourses =
                allCourses?.filter(
                    (course) => course.selection === selection
                ) || [];
            selectionCounts[selection] = selectionCourses.length;
            selectionSamples[selection] = selectionCourses.map((c) => ({
                title: c.title,
        ...
        return c.json({
            ...
            debug: {
                ...
                tableName: "courses",
                availableSelections: [
                    ...new Set(allCourses?.map((c) => c.selection) || []),
                ],
            },
        });
    } catch (error) {
...
app.get("/make-server-9180a2e7/courses", async (c) => {
    try {
        ...
        if (focus) {
            ...
            if (error) {
                console.error(
                    `Database error fetching courses with focus "${focus}":`,
                    error
                );
                return c.json(
                    {
                        success: false,
                        error: "Failed to fetch courses by focus",
                    },
                    500
                );
            }
            ...
            // console.log(
            //     `✅ Found ${enhancedCourses.length} courses with focus "${focus}"`
            // );
        ...
        // console.log(
        //     `✅ Successfully fetched ${enhancedCourses.length} courses from database`
        // );
...
app.get("/make-server-9180a2e7/courses/all", async (c) => {
    try {
        ...
        // console.log(
        //     `✅ Successfully fetched ${enhancedCourses.length} courses for global overlay`
        // );
        return c.json({ success: true, courses: enhancedCourses });
    } catch (error) {
...
app.get("/make-server-9180a2e7/courses/selection/:selection", async (c) => {
    try {
        ...
        if (error) {
            console.error(
                `Database error fetching courses for selection "${selection}":`,
                error
            );
            return c.json(
                {
                    success: false,
                    error: "Failed to fetch courses by selection",
                },
                500
            );
        }
        ...
        // console.log(
        //     `✅ Found ${enhancedCourses.length} courses for selection "${selection}"`
        // );
        return c.json({ success: true, courses: enhancedCourses, selection });
    } catch (error) {
        console.error(
            `Error fetching courses by selection "${selection}":`,
            error
        );
        return c.json(
            { success: false, error: "Failed to fetch courses by selection" },
            500
        );
    }
});
...
app.get("/make-server-9180a2e7/courses/category/:category", async (c) => {
    try {
        ...
        if (error) {
            console.error(
                `Database error fetching courses for category "${category}":`,
                error
            );
            return c.json(
                {
        ...
        // console.log(
        //     `✅ Found ${enhancedCourses.length} courses for category "${category}" (selection: ${selection})`
        // );
        return c.json({ success: true, courses: enhancedCourses, category });
    } catch (error) {
        console.error(
            `Error fetching courses for category "${category}":`,
            error
        );
        return c.json(
            { success: false, error: "Failed to fetch courses by category" },
            500
        );
    }
});
...
app.get("/make-server-9180a2e7/learning-tracks", async (c) => {
    try {
        ...
        return c.json(
            { success: false, error: "Failed to fetch learning tracks" },
            500
        );
    }
});
    ...
    try {
        ...
        if (!newCourse.title || !newCourse.instructor) {
            return c.json(
                {
                    success: false,
                    error: "Missing required fields (title, instructor)",
                },
                400
            );
        }
...
app.get("/make-server-9180a2e7/courses/search/:query", async (c) => {
    try {
        ...
        const { data: courses, error } = await supabase
            ...
            .select("*")
            .or(
                `title.ilike.%${query}%,instructor.ilike.%${query}%,description.ilike.%${query}%`
            )
            .order("created_at", { ascending: false });
        ...
        // console.log(
        //     `✅ Found ${enhancedCourses.length} courses matching "${query}"`
        // );
